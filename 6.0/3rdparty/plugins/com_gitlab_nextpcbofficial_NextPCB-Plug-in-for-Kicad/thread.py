...
class NextPCBThread(Thread):
    ...
    def run(self):
        ...
        bwidth = pcbnew.Iu2Millimeter(board.GetBoardEdgesBoundingBox().GetWidth())
        blength = pcbnew.Iu2Millimeter(board.GetBoardEdgesBoundingBox().GetHeight())
        blayer = board.GetCopperLayerCount()
        ...
        upload_url = baseUrl + '/Upfile/kiCadUpFile'

        self.report(45)

        rsp = requests.post(
            upload_url, files=files, data={'type':'pcbfile','bwidth':bwidth,'blength':blength,'blayer':blayer})
